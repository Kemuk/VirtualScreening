...
if [[ "$chunk_count" -eq 0 ]]; then
    echo "No preparation chunks found in ${chunks_dir}; skipping array submission."
    exit 0
fi
...
array_job_raw=$(sbatch --parsable --array=0-"$array_end" \
    --job-name=vs-preparation-array \
    --time="$time_limit" \
    --output="${slurm_log_dir}/preparation_array_%A_%a.out" \
    --error="${slurm_log_dir}/preparation_array_%A_%a.err" \
    --clusters=arc \
    $partition \
    --export=ALL,CHUNKS_DIR="${chunks_dir}",RESULTS_DIR="${results_dir}",CONFIG_PATH="${config_path}" \
    workflow/slurm/preparation_array.slurm)
